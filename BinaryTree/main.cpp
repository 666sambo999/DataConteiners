...
protected:
	...
	private:
		...
		int depht(Element* Root)const
		{
			...
			T l_depht = depht(Root->pLeft) + 1;
			T r_depht = depht(Root->pRight) + 1;
		...
		void print(Element* Root, T depth, T width)const
		{
			if (Root == nullptr)return;
			width /= 2;
...
template <typename T> 
void measure(const char desctription[], const Tree<T>& tree, T (Tree<T>::* member_function)()const)// desctription - строка массива char
{
...
void main()
{
	...
	//Tree <int> tree = { 50,43,35,26,14,11,7,4,2 };
	Tree <double> tree = { 3.2,6.3,8.5,12.3,16.4,10.9,20.1 };
	tree.print();
	tree.balance();
	tree.true_print();
